 ...
 private <T> T getSplitDetails(Path file, long offset) 
      ...
      if (partitions > 1) {
        ...
        if (melbournekey == 0) {
          ...
          partitioner = new org.apache.hadoop.mapreduce.Partitioner<K,V>() {
            ...
            /**
             * PRNG is a deterministic length-doubling Pseudo Random Number Generator. 
             *
             * Given a 4-byte integer seed 
             * Output 2 pseduorandom 4-byte integers
             * 
             * @return an int array of 2 pseudorandom numbers 
             */ 
            private int[] PRNG(int seed) {
              ...
              // split 8-byte next into two 4-byte ints
              long mask = 0x00000000ffffffffL;
              long first = (next >> 32) & mask;
              long second = next & mask;
            ...
            /**
             * PRF takes in a secret key and an input key.
             * For any attacker who doesn't know the value of this secret key,
             * the output of PRF is indistinguiable from a truly random integer. 
             *
             * Use Goldreich-Goldwasser-Micali(GGM) to 
             * construct a Pseudo Random Function (PRF) from Pseudo Random Number Generator. 
             * 
            ...
            /**
             * The overriden getPartition uses a PRF to assign partition number based on 
             * input <K, V> pairs 
             * 
             * @return an int array of 2 pseudorandom numbers 
             */ 
  ...
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    final ArrayList<SpillRecord> indexCacheList =
      new ArrayList<SpillRecord>(); // a list of spill files
    ...
    public void init(MapOutputCollector.Context context
      ...
      int maxMemUsage = sortmb << 20; // left shift bits; 2^20 = 1 megabyte
    ...
    private void sortAndSpill() throws IOException, ClassNotFoundException,
      ...
      try {
        ...
        sorter.sort(MapOutputBuffer.this, mstart, mend, reporter); // sort the spill record
        int spindex = mstart;
        final IndexRecord rec = new IndexRecord();
        final InMemValBytes value = new InMemValBytes();
        for (int i = 0; i < partitions; ++i) { // for each partition
          IFile.Writer<K, V> writer = null; // create a writer 
          try {
    ...
    private void mergeParts() throws IOException, InterruptedException, 
      ...
      for(int i = 0; i < numSpills; i++) {
        filename[i] = mapOutputFile.getSpillFile(i); // populate filename with the paths for spill files
      ...
      }
      {
        ...
        IndexRecord rec = new IndexRecord();
        final SpillRecord spillRec = new SpillRecord(partitions);
        for (int parts = 0; parts < partitions; parts++) { // for each partition 
          //create the segments to be merged
          List<Segment<K,V>> segmentList =
            new ArrayList<Segment<K, V>>(numSpills); // create a list of segments
          for(int i = 0; i < numSpills; i++) { 
            IndexRecord indexRecord = indexCacheList.get(i).getIndex(parts); // where each segment corresponds to a spill
