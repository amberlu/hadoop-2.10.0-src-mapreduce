...
public class Job extends JobContextImpl implements JobContext, AutoCloseable {
  ...
  // COS518 Edition
  /**
   * Set the secret key of melbourne shuffle for the job. 
   * 
   * @param secret_key user-provided 4-byte integer which serves as the secret key.
   */
  public void setMelbourneShuffle(int secret_key) {
    ensureState(JobState.DEFINE);
    conf.setMelbourneShuffleKey(secret_key); // see mapred.JobConf
  }
  ...
  public boolean waitForCompletion(boolean verbose
                                            ...
                                            ClassNotFoundException {
    if (state == JobState.DEFINE) {
      submit(); // connect to the cluster, then creates a JobSubmitter
    }
    if (verbose) {
      monitorAndPrintJob();
    } else {
